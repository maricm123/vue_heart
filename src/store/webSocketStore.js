...
export const webSocketStore = defineStore('ws', () => {
  ...
  function connectCoach() {
    ...
    wsCoach.value.onmessage = (event) => {
      ...
      if (data.current_calories !== undefined) caloriesFromWsCoach[data.client_id] = data.current_calories
      if (data.bpm !== undefined) bpmsFromWsCoach[data.client_id] = data.bpm
      client[data.client_id] = data.client_id
    }
  ...
  function connectWholeGym() {
    ...
    wsGym.value.onmessage = (event) => {
      ...
      // if (data.client_name !== undefined) client_name[data.client_id] = data.client_name
      // if (data.started_at) {
      //     const parsed = new Date(data.started_at)
      ...
      // ---------- BPM & Calories ----------
      if (data.current_calories !== undefined) caloriesForGym[data.client_id] = data.current_calories
      if (data.bpm !== undefined) bpmsForGym[data.client_id] = data.bpm
      console.log(data.started_at, "DATA STARTED ATTTT")

      // ---------- SECONDS CALC ----------
      if (startedAt[data.client_id]) {
        console.log("USAO U RACUNANJE SECONDS")
        seconds[data.client_id] = Math.floor((Date.now() - startedAt[data.client_id].getTime()) / 1000)
        console.log(seconds[data.client_id])
      }
    }
