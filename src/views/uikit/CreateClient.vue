...
function isPositiveNumber(v) { return v !== null && v !== undefined && !Number.isNaN(Number(v)) && Number(v) > 0 }
...
const isFormValid = computed(() => {
  return (
    ...
    selectButtonValue.value && isNonEmptyString(selectButtonValue.value.name) &&
...
onMounted(() => {
  CountryService.getCountries().then((data) => (autoValue.value = data)).catch(()=>{})
  NodeService.getTreeNodes().then((data) => (treeSelectNodes.value = data)).catch(()=>{})
})
...
function markTouched(name) { if (touched[name]) touched[name].value = true }
</script>
      ...
      <div class="md:w-1/2">
        <div class="card flex flex-col gap-4">
          ...
          <SelectButton
            ...
            optionLabel="name"
            @blur="markTouched('selectButtonValue')"
          />
          ...
          <DatePicker
            ...
            :showIcon="true"
            :showButtonBar="true"
            @blur="markTouched('calendarValue')"
          />
